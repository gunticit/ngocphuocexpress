	...
	<div class="page-header">
		<div class="container-fluid">
			...
			<ul class="breadcrumb">
				...
				<li><a href="<?php echo $breadcrumb['../module/href']; ?>"><?php echo $breadcrumb['text']; ?></a></li>
				<?php } ?>
			</ul>
		</div>
	</div>
					...
					<div class="left">
						<div class="data-block"><!-- category -->  
							...
							<?php foreach( $categories as $category) : ?>
								<li class=""><label><input class="category" type="checkbox" value="<?php echo $category['category_id'];?>" data="<?php echo $category['name'];?>" title="<?php echo $category['title'];?>" /><?php echo $category['name'];?></label></li>
							<?php endforeach;?>
							</ul>
						...
						<div class="data-block"><!-- infomations --> 
							...
							<?php foreach( $informations as $infor) : ?>
								<li class=""><label><input class="infomation"  type="checkbox" value="<?php echo $infor['id'];?>" data="<?php echo $infor['title'];?>" title="<?php echo $infor['title'];?>"/><?php echo $infor['title'];?></label></li>
							<?php endforeach;?>
							</ul>
							<a class="add-to-menu" href="javascript:void(0);">Add To Menu</a>
						</div>
						<div class="data-block"><!-- category -->  
							...
							<?php foreach( $manufacturers as $manufacturer) : ?>
								<li class=""><label><input class="manufacturer"  type="checkbox" value="<?php echo $manufacturer['id'];?>" data="<?php echo $manufacturer['name'];?>" title="<?php echo $manufacturer['name'];?>" /><?php echo $manufacturer['name'];?></label></li>
							<?php endforeach;?>
							</ul>
							<a class="add-to-menu" href="javascript:void(0);">Add To Menu</a>
						</div>
...
function get_content_obj(obj){
	...
	"</div><div class='info hide'>"+
				...
				"<input class='form-control' type='text' name='title[][<?php echo $language['language_id']; ?>]' value='" + jQuery(obj).attr('title') + "'  />" +
