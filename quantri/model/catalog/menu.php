...
class ModelCatalogMenu extends Model {
	...
	public function getOptions(){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "menu_options");
		return $query->rows ? $query->rows : array();
	}
	public function saveOptions($data){
		foreach ($data as $key => $value) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "menu_options WHERE name = '" . $key . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "menu_options SET name = '" . $key . "', value = '" . $this->db->escape($value) . "'");
		}
	}
	...
	public function getMenuHtml($parent_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "menu  WHERE " . DB_PREFIX . "menu.parent_id = '".$parent_id."' ORDER BY " . DB_PREFIX . "menu.id ASC");
		$result=array();
		...
		<li class='dd-item'>
			...
			<?php
				$this->model_catalog_menu->getMenuHtml($value['id']);
			?>
		</li>
		<?php
		endforeach ;
	...
	public function getlist() {
      ...
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "menu  WHERE " . DB_PREFIX . "menu.parent_id = '0' ORDER BY " . DB_PREFIX . "menu.id ASC");
	  $result=array();
	  foreach ($query->rows as $value) {
				$result[] = $value;
	  }	
    ...
    public function install() {
		$query = $this->db->query("CREATE TABLE IF NOT EXISTS ".DB_PREFIX."menu
		(
		...
		$query = $this->db->query("CREATE TABLE IF NOT EXISTS ".DB_PREFIX."menu_options
		(
			id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT,
