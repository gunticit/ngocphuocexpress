	...
	public function refresh($force = false) {
		if (extension_loaded('curl')) {
			...
			if ($force) {
				...
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "currency WHERE date_modified < '" .  $this->db->escape(date('Y-m-d H:i:s', strtotime('-1 day'))) . "'");
			}
			...
			foreach ($lines as $line) {
				if (utf8_substr($line, 4, 3) == $config_currency) {
					$value = (float)utf8_substr($line, 11, 6);
					
					if ((float)$value) $exchange = 1/$value;
			...
			foreach ($lines as $line) {
				...
				$value = $exchange*utf8_substr($line, 11, 6);
