...
class ControllerModuleHTML extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['background2'])) {
			$data['background2'] = $this->request->post['background2'];
		} elseif (!empty($module_info['background2'])) {
			$data['background2'] = $module_info['background2'];
		} else {
			$data['background2'] = '';
		}
		...
		if (isset($this->request->post['background']) && is_file(DIR_IMAGE . $this->request->post['background'])) {
			$data['thumb_bg'] = $this->model_tool_image->resize($this->request->post['background'], 100, 100);
		} elseif ($module_info['background'] && is_file(DIR_IMAGE . $module_info['background'])) {
			$data['thumb_bg'] = $this->model_tool_image->resize($module_info['background'], 100, 100);
		} else {
			$data['thumb_bg'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}

		if (isset($this->request->post['background2']) && is_file(DIR_IMAGE . $this->request->post['background2'])) {
			$data['thumb_bg2'] = $this->model_tool_image->resize($this->request->post['background2'], 100, 100);
		} elseif (!empty($module_info['background2']) && is_file(DIR_IMAGE . $module_info['background2'])) {
			$data['thumb_bg2'] = $this->model_tool_image->resize($module_info['background2'], 100, 100);
		} else {
			$data['thumb_bg2'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
