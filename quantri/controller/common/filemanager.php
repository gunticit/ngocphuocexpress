	...
	public function index() {
		...
		//ocm_CKeditorIsBack_end
				...
				if (in_array($extension, $extraextensions)) {
					$thumb = $this->model_tool_image->resize('ckeditor/' . $extension . "icon.png", 100, 100);
				} else {
			...
			if (!$json) {
				if (!empty($this->request->files['file']['name'][$key]) && is_file($this->request->files['file']['tmp_name'][$key])) {
					...
					$allowed = array(
						...
						'jpeg',
						'gif',
						'png','xls', 'xlsx', 'doc', 'docx', 'ppt', 'pptx', 'odt', 'ods', 'odp', 'zip', 'pdf', 'mp4', 'webm', 'ogv'
					);
