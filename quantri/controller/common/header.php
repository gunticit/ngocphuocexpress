...
class ControllerCommonHeader extends Controller {
	public function index() {
		// Check CKEditor absence in current page.
		function checkCKModAbsence($route) {
			// Mods that prepackage CKEditor.
			$ckmods = array("module/journal2");
			foreach ($ckmods as $ckmod) {
				if (strpos($route, $ckmod) !== false) {
					return false;
				}
			}
			return true;
		...
		$data['ckeditorfull_load'] = isset($this->request->request['route'])?checkCKModAbsence($this->request->request['route']):true;
