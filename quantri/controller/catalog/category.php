...
class ControllerCatalogCategory extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['icon']) && is_file(DIR_IMAGE . $this->request->post['icon'])) {
			$data['thumbicon'] = $this->model_tool_image->resize($this->request->post['icon'], 100, 100);
		} elseif (!empty($category_info) && is_file(DIR_IMAGE . $category_info['icon'])) {
			$data['thumbicon'] = $this->model_tool_image->resize($category_info['icon'], 100, 100);
		} else {
			$data['thumbicon'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
