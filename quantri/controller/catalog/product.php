...
public function delete() {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_date'])) {
			$data['filter_date'] = $sort = $this->request->get['filter_date'];
		} else {
			$data['filter_date'] = $sort = '';
		}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['qdg'])) {
			$data['qdg'] = $this->request->post['qdg'];
		} elseif (!empty($product_info)) {
			$data['qdg'] = $product_info['qdg'];
		} else {
			$data['qdg'] = '';
		}
		...
		if (isset($this->request->post['product_datehour'])) {
			$product_datehours = $this->request->post['product_datehour'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_datehours = $this->model_catalog_product->getProductDateHours($this->request->get['product_id']);
		} else {
			$product_datehours = array();
		}
		$data['product_datehours'] = array();

		foreach ($product_datehours as $product_datehour) {
			$data['product_datehours'][] = array(
				'price'             => $product_datehour['price'],
				'date_product'        => ($product_datehour['date_product'] != '0000-00-00') ? $product_datehour['date_product'] : '',
				'hour_product'          => ($product_datehour['hour_product'] != '00:00') ? $product_datehour['hour_product'] :  ''
			);
		}
		...
		// Desc
		if (isset($this->request->post['product_desc'])) {
			$product_desc = $this->request->post['product_desc'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_desc = $this->model_catalog_product->getProductDesc($this->request->get['product_id']);
		} else {
			$product_desc = array();
		}

		$data['product_desc'] = array();
