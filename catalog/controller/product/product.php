...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['sku'] = $this->model_localisation_zone->getZone($product_info['sku']);

			$data['upc'] = $this->model_localisation_zone->getZone($product_info['upc']);
			...
			$data['qdg'] = !empty($product_info['qdg'])?$product_info['qdg']:'';

			$data['reward'] = $product_info['reward'];
			$data['location'] = $product_info['location'];
			$date_hour = array();
			$datehours = $this->model_catalog_product->getProductDateHour($product_id);
			foreach($datehours as $datehour){
				$date_hour[] = array(
					'datehour' => $datehour['date_product'].'/'.$datehour['hour_product'],
					'price' => VndDot($datehour['price'])
				);
			}
			$data['ean'] = $date_hour;

			$data['points'] = $product_info['points'];
			$data['mtangan'] = html_entity_decode($product_info['mtangan'], ENT_QUOTES, 'UTF-8');
			...
			foreach ($results as $result) {
				...
				$data['images'][] = array(
					...
					//$this->model_tool_image->resize($result['image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_width')),
			...
			$data['video'] = substr(strstr($product_info['sku'],'v='), 2);
