<?php
class Controllerdocumentdocumentcategory extends Controller {
		...
		$this->document->setTitle($this->language->get('heading_title'));
			...
			foreach ($results as $result) {
				...
				if($imglogo == '.docx' || $imglogo == '.doc'){
					$imgl = HTTP_SERVER.'image/format/doc.png';
				}else if($imglogo == '.pdf'){
					$imgl = HTTP_SERVER.'image/format/pdf.png';
				} else if($imglogo == '.xls' || $imglogo == '.xlsx'){
					$imgl = HTTP_SERVER.'image/format/excel.png';
				} else if($imglogo == '.ppt' || $imglogo == '.pptx'){
					$imgl = HTTP_SERVER.'image/format/powerpoint.png';
				...
				// if ($imgl) {
				// 	$thumb = $this->model_tool_image->resize($imgl, 545, 369);
				...
				$data['documents'][] = array(
					...
					'name'        => $result['name'],
					'thumb'       => $imgl,
					'href'        => $this->url->link('document/document', '&document_id='.$result['document_id']),		
				);
			}
			...
			$pagination->url = $this->url->link('document/documentcategory', $url . '&page={page}');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/document/documentcategory.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/document/documentcategory.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/document/documentcategory.tpl', $data));
			}
